...
This is a Rails API project for Giki, a one-stop sustainability platform for SMEs.

The developers behind Giki are the same team behind Exercism and Jiki. If referenced, you can read the Jiki API codebase at `../../jiki/api` and the Exercism website codebase at `../../exercism/website`.
...
Never use `ENV` or Rails credentials directly in application code. Always use the Giki config gem:

- `Giki.config.*` for configuration values (e.g., `Giki.config.frontend_base_url`). Stored in YAML files (`../config/settings/`) for dev/test, DynamoDB for production.
- `Giki.secrets.*` for sensitive values (e.g., `Giki.secrets.stripe_api_key`). Stored in YAML files (`../config/settings/`) for dev/test, AWS Secrets Manager for production.
