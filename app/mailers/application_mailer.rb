class ApplicationMailer < ActionMailer::Base
  ...
  EMAIL_CONFIGS = {
    transactional: {
      from_email: Giki.config.mail_from_email,
      configuration_set: Giki.config.ses_mail_configuration_set
    },
    notifications: {
      from_email: Giki.config.notifications_from_email,
      configuration_set: Giki.config.ses_notifications_configuration_set
    },
    marketing: {
      from_email: Giki.config.marketing_from_email,
      configuration_set: Giki.config.ses_marketing_configuration_set
    }
  }.freeze
  ...
  def mail(**args)
    ...
    args[:reply_to] ||= Giki.config.support_email
  ...
  def unsubscribe_url(token:, key: nil)
    url = "#{Giki.config.frontend_base_url}/unsubscribe?token=#{token}"
