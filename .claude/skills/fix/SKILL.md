...
You are fixing a GitHub issue for the gikiUK/api repository.
...
Read the `CLAUDE.md` file for the area of the codebase involved.
...
Only create the worktree **after the plan is approved**. The issue number has been extracted above (works whether the argument was a full URL like `https://github.com/gikiUK/api/issues/123` or just `123`).
...
- Add or update tests as appropriate (Rails TestUnit, FactoryBot)
...
```bash
bin/rails test <test-file-or-directory>
```
